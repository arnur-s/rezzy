...
- `bg-accent-gradient` (`src/styles.css`) is the utility for app-side solid accent plates. **It currently has no user.** The sidebar's active `WorkspaceMark` was the only one, and that plate was removed when the workspace switcher was aligned to the rail's 16px icon rule. The rail is now fully achromatic, which is what The Near-Neutral Spine Rule asks for — structure takes no hue — but it also means the product has no app-side brand surface at all outside the primary `Button`. Keep the utility, and treat the next surface large enough to carry a ramp as the decision point rather than reintroducing a plate in the rail.
...
- **Workspace switcher:** the first row of the nav body rather than part of the heading. It is a `SideNavItem` — the same component as every other row — with the workspace's curated glyph in the icon slot, the name as the label, and a trailing `chevrons-up-down`. Not a `Button` dressed as a row: the height, inset, radius, hover, pressed, and collapsed behaviour then come from `navItemStyles` rather than from local overrides, which is what keeps it aligned. The list opens in a sibling `Popover` in `anchorRef` mode, because `DropdownMenu` insists on rendering its own `Button` and there is no way to hand it this trigger. Pending invitations put a 6px `bg-error` dot on the icon — on the icon, not in `endContent`, because a collapsed row drops `endContent` and that signal has to survive the collapse.
- **Items:** `SideNavItem` with a 16px Lucide icon. Selection is a quiet accent-tone fill; the same grammar as a conversation row.
- **The 16px icon rule:** every icon in the rail is 16px, and this is a hard constraint rather than a preference. An expanded row left-aligns its icon at a fixed 8px inset while a collapsed row centres it in a fixed 32px box, so an icon of width `w` sits at `8 + w/2` expanded and at `16` collapsed — it holds still across the collapse **only at `w = 16`**. The shift is `(w - 16) / 2`. A 24px plate here moved 4px on every collapse and pushed its label 8px off the rail's text axis; nothing catches this, because it is emergent arithmetic across two centring rules inside Astryx and no token-level scan can see it. An icon slot handed a `ReactNode` also bypasses `renderIconSlot`, so it silently opts out of the row's `size`/`color` contract — pass an `IconType` unless something has to be overlaid on the glyph.
- **Workspace group:** the selected workspace's destinations sit in a nested `SideNavSection` indented `ml-4` behind a `border-l border-border`, which lands the rule on the icon axis every row in the rail shares — 8px of row inset plus half a 16px glyph. The indent is what says the rows belong to that workspace, so the group never repeats the name two rows below the row already showing it — the name goes to the section's hidden group label instead, where a screen reader still gets it. Dropped when collapsed. The bracket runs at full `border-border`.
- **Sections:** three regions. Identity; then the workspace and whatever it contains; then Home and notifications, which span every workspace rather than describing where you are. They are separated by `SideNav`'s own section spacing, not by rules: the shell already divides regions with a gutter, and a horizontal line here would be a third kind of divider in one column.
- **Curated workspace icons:** `WORKSPACE_CURATED_ICONS` must not contain a glyph the rail itself uses for a destination. Since the switcher's icon is now a bare glyph at the same size and tone as the rows beneath it, an overlap is indistinguishable in a collapsed rail. `users-round` was removed for exactly this reason — it is the Contacts row's icon. `resolveWorkspaceIcon` falls back to the default for names no longer on the list, so removing one degrades stored rows rather than breaking them.
